    ...
    <header className="sticky top-0 z-50 border-b border-white/[0.06]"
      ...
      <div className="max-w-screen-2xl mx-auto px-6 flex items-center gap-8 h-14">
        ...
        <nav className="flex gap-1 bg-white/[0.05] rounded-xl p-1 border border-white/[0.06]">
          ...
          {isAdmin && (
            <>
              <div className="w-px h-5 self-center bg-white/[0.12] mx-0.5" />
              <Link href="/admin" title="Administração" aria-label="Administração"
                className={`px-2.5 py-1.5 rounded-lg flex items-center transition-all duration-200 ${
                  pathname.startsWith("/admin") ? "bg-white/10 text-white shadow-sm" : "text-white/40 hover:text-white/70"
                }`}>
